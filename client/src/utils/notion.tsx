...
function parseTableOfContents(block: TableOfContentsBlockObjectResponse) {
  return <div id={block.id} key={block.id}>To Fill</div>
}
...
async function blocksToJSX(
  ...
  // console.log('🥳', tableOfContentsIdx)

  return result
}

export async function getPost(notionClient: Client, slug: string) {
    ...
    const findAndAmendElement = (
      ...
      elements.map(element => {
        if (element.props && element.props.id === targetId) {
          ...
          const newChildren = findAndAmendElement(element.props.children, targetId, newElement)
